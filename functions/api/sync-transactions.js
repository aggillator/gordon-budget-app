...
export async function onRequestPost({ env }) {
  try {
    ...
    for (const item of items) {
      ...
      // Before inserting, check for already-existing CSV-imported rows
      // (synthetic "csv:..." ids) covering the same account+date+amount -
      // import-csv.js only guards the reverse order (CSV after Plaid), so
      // this covers Plaid syncing in after a CSV import already happened.
      // Batched into one lookup query (not one per row) to stay well under
      // Cloudflare's 50-subrequest cap on large first-sync batches.
      const addedRows = [];
      let csvDupMap = new Map();
      let csvRowsToDelete = [];
      if (page.added.length) {
        const dates = page.added.map((t) => t.date);
        const minDate = dates.reduce((min, d) => (d < min ? d : min), dates[0]);
        const maxDate = dates.reduce((max, d) => (d > max ? d : max), dates[0]);
        const candidates = await sb(
          env,
          `transactions?date=gte.${minDate}&date=lte.${maxDate}&plaid_transaction_id=ilike.csv:*&select=id,account_id,date,amount,category_id,category_source,custom_name`
        );
        for (const c of candidates) {
          csvDupMap.set(`${c.account_id}|${c.date}|${c.amount}`, c);
        }
      }

      for (const t of page.added) {
        const account = accounts.find((a) => a.plaid_account_id === t.account_id);
        if (!account || account.sync_disabled) continue;

        let category_id = suggestCategory(rules, t);
        let category_source = category_id ? "auto" : "unassigned";
        let custom_name = null;

        const dupCsv = csvDupMap.get(`${account.id}|${t.date}|${t.amount}`);
        if (dupCsv) {
          if (dupCsv.category_source === "manual") {
            category_id = dupCsv.category_id;
            category_source = "manual";
          }
          custom_name = dupCsv.custom_name;
          csvRowsToDelete.push(dupCsv.id);
        }
      ...
      if (csvRowsToDelete.length) {
        const ids = csvRowsToDelete.map((id) => encodeURIComponent(id));
        await sb(env, `transactions?id=in.(${ids.join(",")})`, { method: "DELETE" });
      }
      if (addedRows.length) {
