...
- **Internal transfer detection**: if you use PayPal funded by a connected card, the same real expense - or its refund - can show up as two transactions: once on PayPal, once on the card. "Find internal transfers" (in Connected accounts) matches same amount (charges and refunds both) + different account + within 2 days, and identifies which side is PayPal by its actual account type (not name-guessing). The PayPal side is excluded by default via the "Internal Transfer" category; the exception is generic funding language ("Add Money", "Amex Send", "Mobile Payment") on the other side, in which case PayPal's side is kept instead since it's the one that actually names the recipient (important for person-to-person transfers). Pairs where neither side is a known PayPal account are left alone rather than guessed. Run it again after future syncs to catch new ones.
