...
- **Internal transfer detection**: if you use PayPal funded by a connected card, the same real expense can show up as two transactions - once on PayPal (paying the merchant) and once on the card (funding PayPal). "Find internal transfers" (in Connected accounts) matches same amount + different account + within 2 days, and excludes one side from the budget via the "Internal Transfer" category. A transaction literally saying "Add Money"/"Amex Send" (pure funding, no recipient info) is the one excluded; otherwise the PayPal-style "Payment to X" entry is excluded in favor of the side with a real merchant name. Ambiguous pairs are left alone rather than guessed - run it again after future syncs to catch new ones.
