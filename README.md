...
The app is organized into two horizontal tab groups instead of a long stack of collapsible sections:
- **Manage** (categories, connected accounts, bank CSV import, Amazon import, action history)
- **View options** (filters, spending chart, category averages)

Budget vs. Actual and the transaction list stay always-visible below both, since those are what you look at most.

## Visual design

A warm gold accent color runs throughout (active tabs, header underlines, highlighted rows, primary buttons), the masthead is a dark navy band with the gold accent as its bottom border, every major section (tab groups, Budget vs. Actual, Transactions) has real card elevation (shadow + rounded corners), and every category gets a consistent color dot/border - same category always gets the same color, computed from its name, no extra data to maintain.
...
- **Merging categories**: each category row has a "Merge into..." dropdown - pick another category and every transaction and keyword rule moves over, budgets combine, and the source category is deleted. Useful for cleaning up near-duplicates (e.g. "Household Goods" and "Household Supplies" both getting created independently over time - AI-created and manually-added categories don't automatically dedupe against each other, so this is the cleanup tool for when that happens).
...
- **Amazon order import**: upload a CSV export of your Amazon order history and it matches item titles to your existing Amazon transactions by amount + date, renaming them via the same `custom_name` field used elsewhere. Column names in Amazon's export have changed over the years, so there's a mapping step - it guesses which CSV column is date/title/amount/order-ID and lets you correct it if the guess is wrong. Matching tries the full order total first, then falls back to matching individual items - Amazon often splits one order into several separate charges as items ship at different times, so the combined total frequently won't match any single real transaction even though each item does. Never overwrites a transaction you've already renamed (manually or from a prior import).
...
- **Internal transfer detection**: if you use PayPal funded by a connected card, the same real expense - or its refund - can show up as two transactions: once on PayPal, once on the card. "Find internal transfers" (in Connected accounts) identifies which side is PayPal by its actual account type (not name-guessing). The PayPal side is excluded by default via the "Internal Transfer" category; the exception is generic funding language ("Add Money", "Amex Send", "Mobile Payment") on the other side, in which case PayPal's side is kept instead since it's the one that actually names the recipient.
- **Refunded purchase netting**: "Find refunded purchases" matches a charge to its own refund on the SAME account (opposite sign, same amount) and excludes both from the budget via the "Refunded Purchase" category - a canceled order no longer counts as spend.
- **Both run automatically after every sync** (and after connecting a new bank) - no need to click the buttons yourself unless you want to re-check on demand. Close, unambiguous matches (a few days apart, no competing candidate) apply automatically. Anything less certain - further apart in time, or tied with another equally-good candidate - gets queued in the **Pending Matches** tab instead of guessed. Each entry shows both transactions and why it wasn't auto-applied; Accept applies it, Reject dismisses it without changing anything. A red count badge on the tab shows how many are waiting.
- **All transactions view**: the month picker now has an "All transactions" option at the top. Budget vs. Actual doesn't apply across an unbounded period (budgets are monthly), so that section points you to the Averages/Trends tabs instead - but the transaction list, search, and filters all work normally across your full history.
- **Uncategorized count badge**: shows in the header whenever you have any uncategorized transactions, across all months. Click it to jump straight to that filtered view.
- **Monthly trends**: "Trends" tab (next to Averages) shows total income, spending, and net per month as a bar chart, plus averages and how the latest month compares to the one before it. Has its own date range filter (defaults to full history) so you can look at any period's average. Uses a dedicated `exclude_from_trends` flag on categories - deliberately separate from `exclude_from_budget`, since those are different questions: "Income" should never appear in the spend-focused Budget view (excluded from budget) but absolutely must count as income here (not excluded from trends). What *is* excluded from trends: internal transfers, refunded purchases, credit card payments (paying your own card from checking isn't new spending - the spending already happened when the card was charged), Maaser, and savings transfers - all real money movement, but between your own accounts, not new economic activity. Toggle either flag per category from "Manage categories" ("Not in budget" / "Not in trends" checkboxes).
- **Budget totals at a glance**: the Budget vs. Actual list now ends with a bold "Total" row - sum of every category's budget cap and sum of actual spend so far this month, with its own progress bar. Same numbers already shown per category, just added up.
- **Tabs collapse on second click**: clicking an already-open tab closes its panel instead of always staying open.
- **Broader AI categorization**: seeded eight general-purpose categories (Electronics, Household Goods, Books & Media, Toys & Games, Health & Personal Care, Office Supplies, Clothing, Home Improvement) so AI categorization of varied purchases (especially Amazon items with real item titles) has good buckets to reach for immediately, and raised the new-category cap per run from 5 to 10.
- **AI categorization now uses the bank's own categorization as a hint**: Plaid provides its own merchant-categorization ("QuickChek" → convenience store, etc.) with every synced transaction, captured in `plaid_category` and fed to the AI prompt as an extra signal - useful for well-known chains the model might not otherwise place confidently.
- **AI decisions now create keyword rules too**, not just manual ones: once a merchant has been categorized the same way by AI 2+ times, that gets promoted to a permanent rule, so future transactions from that merchant are instantly auto-categorized on sync instead of piling up in the unassigned queue waiting for another AI run. Only promotes when AI has been *consistent* about a merchant - if it's categorized the same one differently across runs, no rule gets created from that ambiguity.
